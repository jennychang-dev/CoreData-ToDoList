...
class MasterViewController: UITableViewController, NSFetchedResultsControllerDelegate {
    ...
    func insertNewObject(_ sender: Any) {
        ...
        alertController.addTextField() { textField in
                textField.placeholder = "Priority Level 1-5"
        }
        
        let confirmAction = UIAlertAction(title: "Confirm", style: .default) { [unowned alertController] _ in
            guard let answer = alertController.textFields![0].text, answer != "" else {
                print("error")
                return
            }
            guard let detail = alertController.textFields![1].text, detail != "" else {
                print("error")
                return
            }
            
            guard let priority = alertController.textFields![2].text, priority != "", let priorityAsInt = Int(priority) else {
                print("error")
                return
            }
            
            self.saveToDoList(answer: answer, detail: detail, priorityLevel: priorityAsInt)
        }
    ...
    func saveToDoList(answer: String, detail: String, priorityLevel: Int) {
        ...
        newToDo.priorityNumber = Int16(priorityLevel)
